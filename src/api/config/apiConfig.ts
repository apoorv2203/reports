...
export const apiConfig: Record<string, ApiDefinition> & { host: string; port: string; basePath: string } = {
  ...
  homeWidgets: api('homeWidgets'), widgetData: api('widgetData'), widgetMutations: api('widgetMutations'), pinnedReports: api('pinnedReports'), widgetRecommendations: api('widgetRecommendations'),   widgets: api('widgets'), scheduledDeliveries: api('scheduledDeliveries'), scheduledDeliveryDownload: api('scheduledDeliveryDownload'),
};
