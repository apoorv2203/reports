...
function AppContent() {
  ...
  const toggleHomeWidget = async (id: string) => { const removing = homeWidgetIds.includes(id); const result = await (removing ? removeWidgetFromHome(id) : addWidgetToHome(id)); if (result.widgetId !== id) throw new Error('Unexpected widget mutation response'); const home = await getHomeWidgets(); setHomeWidgets(home.widgets); setHomeWidgetIds(home.widgets.filter((widget) => widget.isOnHome).map((widget) => widget.id)); if (!removing) { setWidgetLoading((current) => ({ ...current, [id]: true })); getWidgetData(id).then((data) => setWidgetData((current) => ({ ...current, [id]: data }))).catch(() => setWidgetErrors((current) => ({ ...current, [id]: true }))).finally(() => setWidgetLoading((current) => ({ ...current, [id]: false }))); } return result; };
