...
export function ParameterFieldsPopover({ reportId, selectedIds, onAdd, children }: Props) {
  ...
  return <div className="w-full">
    ...
    {open && <div className="mt-3 overflow-hidden rounded-lg border border-border bg-white text-foreground shadow-sm">
      <div className="flex items-center justify-between gap-3 p-4"><span className="text-sm font-semibold">{t('reports.addFields')}</span><AppButton variant="ghost" size="icon-sm" onClick={() => setOpen(false)} aria-label={t('reports.closeFieldPicker')}><X /></AppButton></div>
      <div className="flex items-center gap-2 px-4 pb-4"><Search className="size-4 text-muted-foreground" /><AppInput aria-label={t('reports.searchFields')} value={query} onChange={(event) => { setQuery(event.target.value); void loadFields(event.target.value); }} placeholder={t('reports.searchFields')} size="inline" /></div>
