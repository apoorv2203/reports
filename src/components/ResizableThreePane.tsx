...
export function ResizableThreePane({ left, center, right, leftLabel, rightLabel, expandRight }: Props) {
  ...
  return <div className="flex min-h-0 flex-1 overflow-hidden">
    <section style={{ width: leftOpen ? leftWidth : 52 }} className="relative h-full min-h-0 shrink-0 overflow-hidden transition-[width] duration-200">
      <button type="button" onClick={() => setLeftOpen((open) => !open)} aria-label={leftOpen ? t('pane.collapse', { name: leftLabelText }) : t('pane.expand', { name: leftLabelText })} title={leftOpen ? t('pane.collapse', { name: leftLabelText }) : t('pane.expand', { name: leftLabelText })} className="absolute end-3 top-2 z-20 flex size-8 items-center justify-center rounded-md border border-surface-200 bg-white text-ink-500 hover:bg-mint-50">{leftOpen ? <PanelLeftClose className="size-4" /> : <PanelLeftOpen className="size-4" />}</button>
    ...
    <section style={{ width: rightOpen ? rightWidth : 52 }} className="relative h-full min-h-0 shrink-0 overflow-hidden transition-[width] duration-200">
      <button type="button" onClick={() => setRightOpen((open) => !open)} aria-label={rightOpen ? t('pane.collapse', { name: rightLabelText }) : t('pane.expand', { name: rightLabelText })} title={rightOpen ? t('pane.collapse', { name: rightLabelText }) : t('pane.expand', { name: rightLabelText })} className="absolute end-3 top-2 z-20 flex size-8 items-center justify-center rounded-md border border-surface-200 bg-white text-ink-500 hover:bg-mint-50">{rightOpen ? <PanelRightClose className="size-4" /> : <PanelRightOpen className="size-4" />}</button>
