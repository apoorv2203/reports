...
export function ResizableThreePane({ left, center, right, leftLabel = 'Left pane', rightLabel = 'Right pane' }: Props) {
  ...
  return <div className="flex min-h-0 flex-1 overflow-hidden">
    <section style={{ width: leftOpen ? leftWidth : 52 }} className="relative min-h-0 shrink-0 overflow-hidden transition-[width] duration-200">
      <button type="button" onClick={() => setLeftOpen((open) => !open)} aria-label={`${leftOpen ? 'Collapse' : 'Expand'} ${leftLabel}`} title={`${leftOpen ? 'Collapse' : 'Expand'} ${leftLabel}`} className="absolute end-3 top-4 z-10 flex size-8 items-center justify-center rounded-md border border-surface-200 bg-white text-ink-500 hover:bg-mint-50">{leftOpen ? <PanelLeftClose className="size-4" /> : <PanelLeftOpen className="size-4" />}</button>
    ...
    <section style={{ width: rightOpen ? rightWidth : 52 }} className="relative min-h-0 shrink-0 overflow-hidden transition-[width] duration-200">
      <button type="button" onClick={() => setRightOpen((open) => !open)} aria-label={`${rightOpen ? 'Collapse' : 'Expand'} ${rightLabel}`} title={`${rightOpen ? 'Collapse' : 'Expand'} ${rightLabel}`} className="absolute end-3 top-4 z-10 flex size-8 items-center justify-center rounded-md border border-surface-200 bg-white text-ink-500 hover:bg-mint-50">{rightOpen ? <PanelRightClose className="size-4" /> : <PanelRightOpen className="size-4" />}</button>
