...
export function BuilderReportsLibrary({
  ...
  return (
    <div className="rb-overlay fixed inset-0 z-50 flex flex-col overflow-hidden bg-[#f5f5f7]" role="dialog" aria-modal="true" aria-labelledby="reports-library-title">
      <header className="flex min-h-16 shrink-0 flex-wrap items-center justify-between gap-3 border-b border-[#e5e5e7] bg-white px-5 py-3 md:px-8">
        <div className="flex items-center gap-3">
          <button onClick={onBack} aria-label="Back to report builder" className="flex h-9 w-9 items-center justify-center rounded-full text-ink-500 hover:bg-[#f5f5f7] hover:text-ink-900">
      ...
      <main className="min-h-0 flex-1 overflow-y-auto px-5 py-6 md:px-8 md:py-8">
        <div className="mx-auto max-w-7xl">
          <div className="inline-flex rounded-[14px] border border-[#e5e5e7] bg-white p-1 shadow-soft" aria-label="Report library sections">
          ...
          <div className="mt-6 flex flex-col gap-3 lg:flex-row lg:items-center">
            <label className="flex min-w-0 flex-1 items-center gap-3 rounded-[14px] border border-[#d2d2d7] bg-white px-4 py-3 text-ink-300 focus-within:border-mint-400 focus-within:ring-2 focus-within:ring-mint-100">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1" aria-label="Report categories">
              {categories.map((item) => (
                <button key={item} onClick={() => setCategory(item)} className={`shrink-0 rounded-full border px-4 py-2.5 text-[13px] font-semibold ${category === item ? 'border-navy-900 bg-navy-900 text-white' : 'border-[#d2d2d7] bg-white text-ink-500 hover:border-ink-500 hover:text-ink-900'}`}>{item}</button>
              ))}
            </div>
          </div>
          ...
          {visibleReports.length > 0 ? (
            ...
            <div className="mt-6 rounded-[18px] border border-dashed border-[#d2d2d7] bg-white px-6 py-16 text-center">
...
function ReportLibraryCard({ report, onOpen, onFavourite, onPublish }: { report: LibraryReport; onOpen: () => void; onFavourite: () => void; onPublish: () => void }) {
  const categoryStyles: Record<LibraryReport['category'], string> = {
    ...
    Operations: 'bg-[#d9eaff] text-[#174f91]',
  };

  return (
    <article className="flex min-h-[300px] flex-col rounded-[18px] border border-[#e5e5e7] bg-white p-5 shadow-[0_2px_8px_rgba(0,0,0,0.03)] transition hover:-translate-y-0.5 hover:shadow-floaty">
      ...
      <div className="mt-5 flex items-center gap-2">
        {report.ownedByYou ? (
          <>
            <button onClick={onOpen} className="inline-flex flex-1 items-center justify-center gap-2 rounded-[12px] border border-[#d2d2d7] px-3 py-2.5 text-[13px] font-bold text-ink-900 hover:bg-[#f5f5f7]"><FilePenLine className="h-4 w-4" /> Edit</button>
            <button onClick={onPublish} className="inline-flex flex-1 items-center justify-center gap-2 rounded-[12px] border border-[#d2d2d7] px-3 py-2.5 text-[13px] font-bold text-red-700 hover:bg-red-50"><EyeOff className="h-4 w-4" /> {report.published ? 'Unpublish' : 'Publish'}</button>
          </>
        ) : (
          <>
