  ...
  return (
    <div className="mb-5 rounded-[16px] border border-border-strong bg-surface-secondary p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><div className="flex items-center gap-2 text-[12px] font-bold uppercase tracking-[0.12em] text-mint-700"><Filter className="h-4 w-4" /> {t('reports.reportParameters')}</div><p className="mt-1 text-[12px] text-ink-500">{t('reports.setInputs', { reportTitle })}</p></div>
      ...
      <AppButton onClick={runReport} variant="primary" size="action-md">{appliedValues ? <RefreshCw className="h-4 w-4" /> : <Play className="h-4 w-4" />}{appliedValues ? t('reports.runAgain') : t('reports.runReport')}</AppButton>
...
function ParameterField({ parameter, value, error, onChange }: { parameter: ReportParameter; value: string | string[]; error?: string; onChange: (value: string | string[]) => void }) {
  ...
  if (parameter.type === 'single-select') return <label className="flex flex-col gap-1.5">{label}<Select value={String(value)} onValueChange={(next) => next !== null && onChange(next)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="All">All</SelectItem>{parameter.options?.map((option) => <SelectItem key={option} value={option}>{option}</SelectItem>)}</SelectContent></Select>{error && <span className="text-[11px] text-red-600">{error}</span>}</label>;
  return <label className="flex flex-col gap-1.5">{label}<AppInput type={parameter.type === 'number' ? 'number' : parameter.type === 'date' ? 'date' : 'text'} value={String(value)} onChange={(event) => onChange(event.target.value)} />{error && <span className="text-[11px] text-red-600">{error}</span>}</label>;
}
