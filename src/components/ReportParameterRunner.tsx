...
export function ReportParameterRunner({ parameters, reportTitle }: { parameters: ReportParameter[]; reportTitle: string }) {
  ...
  function runReport() {
    console.log('ReportParameterRunner.runReport called', { values });
    ...
    if (Object.keys(nextErrors).length === 0) {
      ...
      console.log('ReportParameterRunner.appliedValues', applied);
    } else {
      console.log('ReportParameterRunner.validationErrors', nextErrors);
    }
  }
    ...
    <div className="mb-5 rounded-[16px] border border-border-strong bg-muted p-4">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        {parameters.map((parameter) => <ParameterField key={parameter.id} parameter={parameter} value={values[parameter.id]} error={errors[parameter.id]} onChange={(value) => setValues((current) => ({ ...current, [parameter.id]: value }))} />)}
          ...
          <AppButton onClick={runReport} variant="reportRun" size="action-md" className="bg-black text-white hover:bg-black/90">{appliedValues ? <RefreshCw className="h-4 w-4" /> : <Play className="h-4 w-4" />}{appliedValues ? t('reports.runAgain') : t('reports.runReport')}</AppButton>
