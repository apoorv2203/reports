    ...
    <div className="fixed inset-0 z-50 flex flex-col overflow-hidden bg-card">
      <header className="flex h-14 shrink-0 items-center justify-between border-b border-border bg-card px-5 text-foreground shadow-sm">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <span className="h-5 w-px bg-card-200" />
      ...
      <ResizableThreePane
        ...
        left={
          <aside className={`flex h-full min-h-0 flex-col border-r border-border bg-background p-4 ${readOnly ? 'items-center justify-center' : ''}`}>
            ...
            {!readOnly && <div className="mt-4 flex-1 overflow-y-auto">
              ...
              <div className="mt-6 border-t border-border pt-4">
                <div className="text-[12px] font-bold text-muted-foreground">Try next</div>
        ...
        right={
          <aside className="flex h-full min-h-0 flex-col gap-3 overflow-y-auto border-l border-border bg-background p-4">
            ...
            <div className={readOnly ? 'hidden' : 'text-[12px] font-bold uppercase tracking-wide text-muted-foreground'}>Layout</div>
...
function ReportSectionCard({ section, onEdit, readOnly = false }: { section: TemplateSection; onEdit: (value: string) => void; readOnly?: boolean }) {
  ...
  return <section className={`relative rounded-lg border p-5 ${section.kind === 'empty' ? 'border-dashed border-border-light bg-card-tertiary' : 'border-border bg-card shadow-card-alt'}`}>
    <div className="flex items-center justify-between"><div className="flex items-center gap-2 text-[12px] font-bold uppercase tracking-wide text-muted-foreground"><Icon className="h-4 w-4 text-primary" /> {section.kind === 'empty' ? 'Empty section' : section.kind}</div>{!readOnly && <AppButton size="section-icon" aria-label="Edit section" onClick={() => setEditing(!editing)}><Pencil /></AppButton>}</div>
    {editing ? <AppInput autoFocus defaultValue={section.title} onBlur={(e) => { onEdit(e.target.value); setEditing(false); }} onKeyDown={(e) => { if (e.key === 'Enter') { onEdit(e.currentTarget.value); setEditing(false); } }} className="mt-3" /> : <h3 className="mt-3 font-display text-[18px] font-bold tracking-[-0.03em] text-foreground">{section.title}</h3>}
    ...
    {section.kind === 'empty' && <div className="mt-5 flex min-h-[72px] items-center justify-center text-center text-[13px] text-muted-foreground">Empty — ask ReportIQ to add a chart, table, or metric here</div>}
  </section>;
}
...
function MiniTable() { return <div className="mt-4 overflow-hidden rounded-[10px] border border-border"><div className="grid grid-cols-3 bg-navy-900 px-3 py-2 text-[11px] font-bold text-white"><span>Segment</span><span>Accounts</span><span className="text-right">Balance</span></div>{['Retail', 'Corporate', 'SME'].map((row, index) => <div key={row} className="grid grid-cols-3 border-t border-border px-3 py-2 text-[12px] text-foreground"><span>{row}</span><span>{[782, 341, 161][index]}</span><span className="text-right font-semibold">{['43.2 Cr', '31.8 Cr', '22.9 Cr'][index]}</span></div>)}</div>; }
