...
function ReportSectionCard({ section, onEdit, readOnly = false }: { section: TemplateSection; onEdit: (value: string) => void; readOnly?: boolean }) {
  ...
  return <section className={`relative rounded-lg border p-5 ${section.kind === 'empty' ? 'border-dashed border-border-light bg-surface-tertiary' : 'border-surface-200 bg-surface shadow-card-alt'}`}>
    <div className="flex items-center justify-between"><div className="flex items-center gap-2 text-[12px] font-bold uppercase tracking-wide text-ink-500"><Icon className="h-4 w-4 text-mint-600" /> {section.kind === 'empty' ? 'Empty section' : section.kind}</div>{!readOnly && <AppButton size="section-icon" aria-label="Edit section" onClick={() => setEditing(!editing)}><Pencil /></AppButton>}</div>
